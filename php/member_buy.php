	...
	if(isset($_GET['count']) && $_SESSION['user_id'])
	{
		...
		if($result = mysqli_fetch_array(mysqli_query($conn, $sql_find)))
		{
			...
			if($result_ticket_code = mysqli_fetch_array(mysqli_query($conn, $sql_find_ticket)))
			{
				...
				{
					echo "<script>alert('{$count}개의 식권을 구매하였습니다.');location.href='../';</script>";
				}
				else
				{
					echo "<script>alert('식권 구매에 실패했습니다. 잠시후 다시 시도해주세요.');location.href='../member_buy.html';</script>";
				}
			}
			else
			{
		...
		{
			echo "<script>alert('잘못된 접근입니다.');location.href='../member_buy.html';</script>";
		}
	}
	else
	{
